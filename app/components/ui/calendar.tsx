...
// react-day-picker's own default class names carry the day-cell
// selection/range logic; this file only adds Tailwind classes on top.
// The shadcn "Nova" preset generates this component against Tailwind
// v4's CSS-native features (--spacing()/var()-from-css-var shorthand
// like size-(--cell-size), the in-data-[slot=...]: variant, the **:
// all-descendants variant for RTL). None of those exist in Tailwind
// v3, so every one is rewritten below to its v3 equivalent (explicit
// var() inside bracket arbitrary values, or dropped outright for
// features this LTR-only app doesn't need).
function Calendar({
  className,
  classNames,
  ...
  return (
    <DayPicker
      ...
      className={cn(
        "group/calendar bg-background p-2 [--cell-radius:calc(var(--radius)_-_2px)] [--cell-size:1.75rem]",
        className
      )}
...
function CalendarDayButton({
  ...
  return (
    <Button
      ...
      className={cn(
        "relative isolate z-10 flex aspect-square size-auto w-full min-w-[var(--cell-size)] flex-col gap-1 border-0 leading-none font-normal group-data-[focused=true]/day:relative group-data-[focused=true]/day:z-10 group-data-[focused=true]/day:border-ring group-data-[focused=true]/day:ring-2 group-data-[focused=true]/day:ring-ring/50 data-[range-end=true]:rounded-[var(--cell-radius)] data-[range-end=true]:bg-primary data-[range-end=true]:text-primary-foreground data-[range-middle=true]:rounded-none data-[range-middle=true]:bg-muted data-[range-middle=true]:text-foreground data-[range-start=true]:rounded-[var(--cell-radius)] data-[range-start=true]:bg-primary data-[range-start=true]:text-primary-foreground data-[selected-single=true]:bg-primary data-[selected-single=true]:text-primary-foreground [&>span]:text-xs [&>span]:opacity-70",
        defaultClassNames.day,
        className
      )}
      {...props}
